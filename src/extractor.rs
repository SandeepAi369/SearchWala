// ============================================================================
// Swift Search Agent - Content Extractor
// ============================================================================
...
const STRIP_TAGS: &[&str] = &[
    ...
    "select", "textarea", "iframe", "noscript", "svg", "path", "canvas",
];
...
pub fn extract_article_text(html: &str) -> String {
    ...
    // Final fallback: body text with aggressive cleanup.
...
fn try_scored_extraction(doc: &Html) -> Option<String> {
    ...
    for element in doc.select(&container_sel) {
        ...
        if score > best_score {
            best_score = score;
            best_text = extract_visible_text(&element);
        }
    }
...
fn extract_visible_text(element: &ElementRef) -> String {
    ...
    let skip_tags: std::collections::HashSet<&str> = [
        "script", "style", "nav", "header", "footer", "aside", "form", "button", "iframe", "noscript", "svg",
    ]
    .iter()
    .cloned()
    .collect();
