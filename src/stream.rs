// ============================================================================
// SearchWala v5.0.1 - Streaming Search (SSE)
...
pub fn execute_stream_search(
    ...
    tokio::spawn(async move {
        ...
        let _ = tx.send(Ok(Event::default().data(serde_json::json!({"type": "info", "text": "Starting SearchWala Meta-Search Pipeline..."}).to_string()))).await;
        ...
        if enable_copilot.unwrap_or(false) {
            if let Some(cfg) = &llm_config {
                let _ = tx.send(Ok(Event::default().data(serde_json::json!({"type": "info", "text": "SearchWala Copilot rewriting query..."}).to_string()))).await;
